            ...
            <div class="container">
                ...
                <form class="form-horizontal" method="post">
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <input type="text" class="form-control" name="username" id="username" value="${requestScope["users"].getUserName()}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-2">Password</label>
                        <div class="col-sm-7">
                            <input type="password" class="form-control" name="password" id="password" value="${requestScope["users"].getPassword()}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-2">Fullname</label>
                        <div class="col-sm-7">
                            <input type="text" class="form-control" id="fullname" name="fullname" value="${requestScope["users"].getFullName()}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-2">Trạng thái:</label>
                        <div class="col-sm-7">
                            <select class="form-control" name="status" id="status value="${requestScope["users"].getStatus()}"">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <select class="form-control" name="roleid" id="roleid" value="${requestScope["users"].getRoleId()}">
                                <option value="1">Admin</option>
                                <option value="2">Người dùng</option>
                            </select>

<%--                            <select name="roleid" id="roleid" value="${userModel.getRoleId()}">--%>
<%--                                <option value="">ALL</option>--%>
<%--                                <c:forEach items="${requestScope['users'].getRoleId()}" var="role">--%>
<%--                                    <option value="${role}" ${userModel.getRoleId() == role ? 'selected' : ' '}><c:out value="${role}" ></c:out></option>--%>
<%--                                </c:forEach>--%>
<%--                            </select>--%>
<%--                                <select name='role'>--%>
<%--                                    <option value="${selected}" selected>${selected}</option>--%>
<%--                                        <c:if test="${requestScope['users'].getRoleId() != selected}">--%>
<%--                                            <option value="${role}">${role}</option>--%>
<%--                                        </c:if>--%>
<%--                                </select>--%>

<%--                            <select name="roleid" id="roleid" value="${requestScope["users"].getRoleId()}">--%>
<%--                                <option value="">ALL</option>--%>
<%--                                <c:forEach items="${requestScope['users'].getRoleId()}" var="role">--%>
<%--                                    <option value="${role}" ${requestScope['users'].getRoleId() == role ? 'selected' : ' '}>--%>
<%--                                        <c:if test="${role == '1'}">Admin</c:if>--%>
<%--                                        <c:if test="${role == '2'}">Người dùng</c:if>--%>
<%--                                    </option>--%>
<%--                                </c:forEach>--%>
<%--                            </select>--%>
                        </div>
